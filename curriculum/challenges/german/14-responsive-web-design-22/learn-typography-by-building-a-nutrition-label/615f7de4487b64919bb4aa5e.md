...
Erstelle nach deinem letzten `.divider` ein neues `p`-Element mit dem Text `Cholesterol 0mg 0%`. Umschließe den Text `Cholesterol` in ein `span`-Element und teile dem `span`-Element das `class`-Attribut zu, das auf `bold` gesetzt ist. Umschließe den Text `0%` mit einem weiteren `span`-Element, wobei `class` auf `bold` gesetzt ist. Zum Schluss verschachtelst du `Cholesterol` und `0mg` `span`-Elemente in einem zusätzlichen `span`-Element zur Ausrichtung.
